    ...
    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
      <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
        <tr class="bg-white">
          ...
          <th scope="col" class="px-4 py-4 text-left">{{ t('email') }}</th>
      ...
      <tbody>
        ...
        <tr v-else-if="!isLoading && userList.length === 0"
          class="bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700 h-80">
          <td colspan="5">
            <div class="flex justify-center">
        ...
        <tr v-else v-for="(item, index) in userList" :key="index"
          ...
          <td class="px-4 py-4 text-center text-gray-900">
            <button class="m-1" size="xs" gradient="gray" @click="editUser(item.id)">
              <Icon name="mynaui:edit-one" size="25" />
            </button>
            <button class="m-1" size="xs" gradient="gray" @click="deleteUser(item.id)">
